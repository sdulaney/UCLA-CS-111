...
time_t next_time = 0;
int logfd = 0;
struct pollfd poll_commands;
poll_commands.fd = 0;
poll_commands.events = POLLIN;
poll_commands.revents = 0;
...
void process_commands() {
    ssize_t rv = read(0, &command_buf, 511);
    if (rv == -1) {
...
int main(int argc, char ** argv) {
    ...
    while (1) {
	print_report();
	int rv_poll = poll(&poll_commands, 1, 0);
	if (rv_poll == -1) {
	    fprintf(stderr, "Error polling for commands on STDIN.\n");
	    exit(1);
	}
	else if (rv_poll > 0 && (poll_commands.revents & POLLIN)) {
	    process_commands();
	}
    }
